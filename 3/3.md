...
> str.zfill(width): 返回原字符串的副本，在左边填充 '0' 字符使其长度变为 width。 正负值前缀 ('+'/'-') 的处理方式是在正负符号之后 填充而非在之前。 如果 width 小于等于 len(s) 则返回原字符串的副本。
...
>>> "-42".zfill(5)
'-0042'
>>> "0b0101".zfill(8) # ! 0b 前缀在这里当作普通字符处理，想二进制补零时要把 0b 前缀去掉
'000b0101'
```
